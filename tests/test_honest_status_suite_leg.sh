...
# (MF review port 2026-07-28: the empty-summary branch ran first and
# downgraded an OOM-killed suite from FAIL to UNKNOWN. A nonzero exit is real
# evidence — the measured flap only LOSES failures toward 0, never invents
# nonzero.)
out="$(run 137 '')"
...
# ── INTERNALERROR display honesty (MF review port 2026-07-28) ────────────
...
# an unanchored count flipped a green run to bad on display noise.
