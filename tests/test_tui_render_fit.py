...
PORTED FROM MESHFORGE (2026-09-16). MeshAnchor carried the ORIGINAL
shrink-only fit, so it had TWO defects at once:

  1. A multi-line text panel inside the fixed 22-row box was clipped even
     on a tall terminal. MeshForge fixed that half in review F3 and the
     fix never ported here.
  2. The list never grew: `lh` stayed at the default 14 however tall the
     terminal, so a 22-item menu scrolled identically on a 60-row
     terminal that could show every row with space to spare.

MeshForge measured (2) with scripts/tui_smoke.py against real whiptail —
list_height read 14 at 24, 30, 40 AND 60 rows — and the one-line cause was
`needed` being computed from the DEFAULT list_height instead of
len(choices). Swept against MeshAnchor's own old formula: 5,754
identical, 6,542 changed, ZERO regressions.
