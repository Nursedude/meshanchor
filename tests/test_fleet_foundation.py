...
drift detection (owned/root-owned/absent), idempotent plan, and the MeshAnchor
...
from utils.fleet_foundation import (
    ...
    meshanchor_data_roots,
    plan_data_root_fixes,
)

HOME = Path("/home/op")
ROOTS = (HOME / ".config" / "meshanchor", HOME / ".cache" / "meshanchor")
...
class TestCanonicalFoundation:
    ...
    def test_data_roots_are_the_meshanchor_dirs(self):
        spec = canonical_foundation(operator_user="op", home=HOME)
        assert spec.data_roots == meshanchor_data_roots(HOME)


class TestMeshanchorDataRoots:
    def test_includes_xdg_dirs_and_install_dir(self):
        roots = meshanchor_data_roots(HOME)
        assert HOME / ".config" / "meshanchor" in roots
        assert HOME / ".local" / "share" / "meshanchor" in roots
        assert HOME / ".cache" / "meshanchor" in roots
        # The install dir — the daemon writes logs/db under it (canary finding).
        assert Path("/opt/meshanchor") in roots

    def test_does_not_include_etc_reticulum(self):
        # The RNS tree is the shared/parity core (rns_tree_perms), not a data-root.
        assert Path("/etc/reticulum") not in meshanchor_data_roots(HOME)
