...
class TestDeduplication:
    ...
    def test_identical_content_same_millisecond_gets_distinct_ids(
        ...
        """Regression (the test_dedup_disabled flake): the ms-timestamp +
        content-hash id was NOT unique — identical content enqueued twice
        in the same millisecond collided on the TEXT PRIMARY KEY and the
        second INSERT failed. Clock frozen so the collision shape is
        deterministic, not timing-dependent. (Mirror of MeshForge.)"""
...
# ---------------------------------------------------------------------------
# PersistentMessageQueue — per-destination TX pacing (2026-06-04, MF parity)
# ---------------------------------------------------------------------------
# meshtasticd 2.7.x NAKs burst API text broadcasts with Routing.Error
# RATE_LIMIT_EXCEEDED (=38) while the toradio/MQTT hand-off reports success —
# a multi-chunk RNS→Mesh message dispatched back-to-back silently loses every
# chunk after the first on RF (MeshForge live find, 2026-06-04).
# register_sender(min_spacing_s=...) paces the dispatch loop: not-yet-due
# messages stay 'pending' for a later process_once pass.
...
# ---------------------------------------------------------------------------
# 2026-07-09 frontier review Pri-2 fixes (ported from MeshForge)
# ---------------------------------------------------------------------------

class TestFrontierReviewFixes20260709:
    """Pins for the message_queue findings of the 2026-07-09 frontier
    worklist Pri-2 pass (MeshForge review_provenance.md): shed messages
    leave a QUEUE_SHED witness, mark_acked has exactly one winner under
