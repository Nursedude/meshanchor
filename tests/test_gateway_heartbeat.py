...
class TestEventHistoryThreadSafety:
    """Pri-4 (2026-07-23 review, MF twin): _events is written from the MQTT
    callback thread, the per-detection daemon threads, and read by get_status —
    all with no lock. The `self._events = self._events[-max:]` rebind is a
...
class TestClockStepDoesNotSplitBrain:
    """Regression pins for the 2026-07-23 gateway review, Pri-1 (ported from MF).
    ...
    def test_negative_relative_mono_is_aged_not_skipped(self, secondary_config):
        ...
        # the unset 0.0 sentinel, not a negative stamp (a legitimately old peer).
        hb = GatewayHeartbeat(config=secondary_config)
    ...
    def test_unset_mono_sentinel_is_skipped(self, secondary_config):
        # The 0.0 default (alive peer with no monotonic stamp) must NOT forge a
        # down from now_mono - 0.
        hb = GatewayHeartbeat(config=secondary_config)
