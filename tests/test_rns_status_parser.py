...
FULL_OUTPUT = """\
 Shared Instance[rns/meshanchor fleet-host-2 rns]
    Status    : Up
    Serving   : 0 programs
    Rate      : 1.00 Gbps
    Traffic   : ↑242 B  0 bps
...
class TestParseDisplayNames:
    def test_shared_instance_name(self):
        result = parse_rnstatus(FULL_OUTPUT)
        assert result.interfaces[0].display_name == "rns/meshanchor fleet-host-2 rns"
    ...
    def test_full_name_property(self):
        result = parse_rnstatus(FULL_OUTPUT)
        assert result.interfaces[2].full_name == "TCPInterface[Regional RNS/192.168.86.38:4242]"
...
class TestRunRnstatusEmptyOrFailedIsNotClean:
    """Falsifiability phase 2 (2026-09-02, ported from MeshForge): ``rnstatus``
    that prints NOTHING, or exits non-zero with text matching no known error
    pattern, came back as a bare RNSStatus with no parse_error — and the
