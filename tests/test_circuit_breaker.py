...
class TestMonotonicClockIssue74:
    """MF Issue #74 port: recovery-elapsed math must run on
    time.monotonic(). Pi fleet boxes NTP-step after boot; with
    wall-clock math a backward step froze an OPEN circuit past
    recovery_timeout and a forward step recovered it prematurely."""

    def _fake_clocks(self):
        """Controllable monotonic + wall clocks."""
        return {"mono": 1000.0, "wall": 5_000_000.0}
    ...
    def test_half_open_second_call_blocked(self):
        ...
        (the transitioning caller takes the slot — off-by-one found
        by the MF #74 review)."""
        cb = CircuitBreaker(
