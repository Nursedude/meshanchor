...
PORTED FROM MESHFORGE 2026-09-24 (`tests/test_tui_success_truth_sweep.py`
there, at 3221353b's tree). The instrument is shared; the findings are this
app's own. MA deltas: /etc/meshanchor, /var/lib/meshanchor, /run/meshanchor
are box state; the dump env var is MA_TRUTH_SWEEP_DUMP; the frozen
baselines below are MeshAnchor's first walk, not MeshForge's. The history
notes (Fable reviews, dated catches) are MeshForge's — they are why each
leg exists, and they are kept so the next reader does not re-learn them.
...
Run with MA_TRUTH_SWEEP_DUMP=<path> to write every action's verdict and
...
def test_level_two_items_tell_the_truth(section, tag, dead_externals):
    ...
    the MA_TRUTH_SWEEP_DUMP `.level2` file but NOT gated: at level two it
...
def test_known_crashed_is_frozen_not_just_documented():
    ...
    # MeshAnchor's first walk (2026-09-24) fixed every crasher it found
    # instead of baselining it, so both sets are frozen EMPTY here.
    frozen: set = set()
...
def _write_dump():
    yield
    path = os.environ.get("MA_TRUTH_SWEEP_DUMP")
    if path and _DUMP:
