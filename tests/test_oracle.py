...
# NOTE: MeshForge pins the oracle's stale thresholds to its
# _map_status_endpoints.py _WATCHDOG_STALE_S/_MINI_STALE_S readers. MeshAnchor's
# status endpoint is leaner (no watchdog/mini-dudeai stitch — its watchdog is
# SQLite blackout rows, and there is no mini-dudeai), so there is no canonical
# reader to pin against; the oracle's thresholds stand alone here. Test omitted
# deliberately (not forgotten). See the MeshAnchor port notes.
...
def test_oracle_log_path_under_data_dir_not_bare_home():
    ...
    assert p.parent == MeshAnchorPaths.get_data_dir()
