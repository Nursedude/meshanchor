...
PORTED from MeshForge 8910c156 (2026-09-18). ⚠️ This is the repo where the
leak is LIVE: meshcore.enabled is true on meshanchor-server, while MeshForge
has MeshCore disabled on every box — so the fix was built and proven there
and lands HERE as the one that changes real behaviour.
...
def _no_env(monkeypatch):
    ...
    monkeypatch.delenv("MESHANCHOR_MESHCORE_BRIDGE_CHANNELS", raising=False)
...
class TestPredicateExplicit:
    ...
    def test_env_list_is_honoured_exactly(self, monkeypatch):
        monkeypatch.setenv("MESHANCHOR_MESHCORE_BRIDGE_CHANNELS", "0,2")
        h = _policy()
    ...
    def test_empty_env_means_bridge_nothing_not_bridge_all(self, monkeypatch):
        ...
        """
        monkeypatch.setenv("MESHANCHOR_MESHCORE_BRIDGE_CHANNELS", "")
        h = _policy()
        for chan in (0, 1, 5):
    ...
    def test_env_overrides_declared_config(self, monkeypatch):
        monkeypatch.setenv("MESHANCHOR_MESHCORE_BRIDGE_CHANNELS", "9")
        h = _policy(_make_config(bridge_source_channels=[3]))
    ...
    def test_typo_is_loud_and_does_not_widen_the_gate(self, monkeypatch):
        monkeypatch.setenv("MESHANCHOR_MESHCORE_BRIDGE_CHANNELS", "1,oops")
        h = _policy()
