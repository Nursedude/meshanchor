...
class TestReconnectStrategyIntegration:
    ...
    def test_wait_returns_actual_delay(self, strategy):
        ...
        MF Issue #74 port: wait() is Event.wait-based now (no
        time.sleep) — patch the never-set fallback event with a
        pre-set one so the test is instant while still asserting the
        computed delay."""
        import threading
...
class TestWaitNeverSleepsIssue74:
    """MF Issue #74 port: ReconnectStrategy.wait() must always be
    Event.wait-based — the old bare time.sleep fallback (no
    stop_event) slept uninterruptibly up to max_delay (60s) on
    shutdown."""
    ...
    def test_wait_source_contains_no_time_sleep(self):
        ...
        assert "time.sleep" not in src, (
            "wait() regressed to a bare time.sleep — MF010-class; use "
            "the never-set Event fallback"
        )
