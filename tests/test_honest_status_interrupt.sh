...
#
# Ported from MeshForge 2026-09-07 (lead repo); this twin carried the same
# trap verbatim. The trap was `trap 'rm -rf "$HS_TMP"' EXIT INT TERM` — a handler
...
PATH="$SB:$PATH" HOME="$FAKE_HOME" XDG_STATE_HOME="" HONEST_BOXES="hs-test-dummy" \
  MESHANCHOR_REPO="$FAKE_REPO" setsid bash "$SCRIPT" > "$out" 2>&1 &
pid=$!
