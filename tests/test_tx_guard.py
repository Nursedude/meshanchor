...
Ported from MeshForge (the lead repo for this arc) — same bridge lineage,
same fallback chain, same exposure.
...
class TestSocketTripwire:
    ...
    def test_connect_ex_probe_idiom_is_permitted_and_recorded(self):
        ...
        production sites were refactored to stop causing."""
        tx_guard.clear_probe_attempts()
        s = socket.socket()
        try:
...
class TestRnsEgress:
    ...
    def test_gateway_announce_site_refuses(self):
        ...
        _safe_announce runs on the LXMFBroadcast-Announce daemon thread, so
        its guard catch is DELIBERATE (see tx_guard docstring, MF finding-5
        class): a refusal must NOT announce and must leave the recorded
        witness — and must not raise, or the announce thread dies."""
        from gateway.lxmf_broadcast_bridge import LXMFBroadcastBridge
        b = LXMFBroadcastBridge.__new__(LXMFBroadcastBridge)
        b._destination_hash = b"\x01" * 16
        b._router = MagicMock()
        b._safe_announce()  # must not raise
        b._router.announce.assert_not_called()
        recs = tx_guard.blocked_attempts()
        assert recs and recs[-1]["kind"] == "rns_announce"
