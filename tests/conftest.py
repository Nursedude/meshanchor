...
def _reset_node_tracker_singleton():
    ...
    The bridge now SHARES the ``get_node_tracker()`` singleton instead of
    building a private ``UnifiedNodeTracker`` (2026-09-22, the fix for the
    shutdown clobber: two trackers over one node_cache.json, the stale one
    flushing last). Without this reset the second test to construct a bridge
    would inherit the first test's node set — cross-test leakage that surfaces
    as a flake in whichever test reads the population next, nowhere near the
    test that dirtied it.
    ...
    Runs on setup as well as teardown: a test that leaks despite this (an
    xdist crash, a fixture that errors) must not poison its successor.
    """
    def _reset():
