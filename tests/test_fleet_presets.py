...
catalog the TUI fleet-provision feature reads.

A preset must reference a REAL role (in fleet_roles.yaml) and REAL bridge legs
(in its own `legs:` vocabulary) — otherwise the TUI would offer a variation that
provision_role.py can't apply. This is the closed-consumer guard
(honest_failure_modes #7): the catalog and the role SSOT are validated together,
not independently. Ported from MeshForge alongside the provisioner.
...
def test_bridge_legs_are_known_and_only_when_bridging(presets_doc):
    ...
    carry them (a monitor/noc preset runs no bridge). MeshAnchor's bridge is a
    ...
    for name, p in presets_doc["presets"].items():
        ...
        assert len(legs) == 1, \
            f"bridge preset {name!r} must carry exactly one leg (bridge_mode is " \
            f"a single enum), got {legs}"
...
def test_singletons_match_role_singleton(presets_doc, role_names):
    ...
    fleet_roles.yaml (noc) — keeps the catalog honest about the
    one_canonical_noc invariant."""
    with open(ROLES_PATH) as f:
...
def test_leg_gateway_json_flags_present(presets_doc):
    """Each leg declares the gateway.json flag(s) it sets (bridge_mode) so the
    handler can translate a preset to a gateway.json overlay deterministically."""
    for leg, spec in presets_doc["legs"].items():
        ...
        assert "bridge_mode" in spec["gateway_json"], \
            f"leg {leg!r} must set a bridge_mode (MeshAnchor's single-enum bridge)"
