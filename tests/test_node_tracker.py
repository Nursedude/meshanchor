...
class TestCacheWriteChurn20260803:
    ...
    def test_stop_does_not_flush_when_never_started(self, tracker):
        ...
        Measured on meshanchor-server 2026-09-22: the daemon ran TWO trackers
        over one node_cache.json. The bridge's grew from RX; the singleton sat
        on the snapshot it loaded at startup and, stopping LAST, flushed that
        over the live file — 77 nodes (rns 57 / meshcore 20) became 52
        (rns 38 / meshcore 14), newest last_seen 31 minutes backward, roughly
        four times a day on the restart timer.

        The structural cure is one shared tracker (see
        test_bridge_shares_the_singleton_tracker). This is the invariant that
        makes the class impossible to reintroduce: an instance that never ran
        the live loop has nothing authoritative to write. Eviction cannot cure
        this direction — it removes expired nodes, it cannot add back ones
        this instance never heard.
        """
        t, cache_file, _ = tracker
        cache_file.unlink(missing_ok=True)
        t.stop(timeout=0.1)
...
class TestPopulationRetention20260803:
    ...
    def tracker(self, tmp_path):
        with patch.object(UnifiedNodeTracker, '_load_cache'):
            ...
            # entirely (the clobber fix), so without this they would be
            # asserting against a stop() that correctly does nothing.
            t._ever_started = True
            yield t
