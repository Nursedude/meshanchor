...
# Source directory.
#
# WARNING: this MUST be absolute/normalised. Un-normalised, it reads
# `<repo>/tests/../src`, so every path produced by `_scan_python_files`
# literally contained `/tests/` — and the guards below skip a match with
# `if 'test_' in basename or '/tests/' in filepath`. That skip therefore
# discarded EVERY match, and those guards could not fail no matter what
# landed in src/. Proven by live drill in MeshForge 2026-08-05: a file added
# to src/ with a bare `shell=True` passed TestNoShellTrue. Layer 1
# (scripts/lint.py) still caught it, so the fleet was not unprotected — but
# every Layer-2 contract without a lint twin was inert.
#
# `TestGuardsAreNotInert` pins that the scanner can actually see src/.
...
def _strip_trailing_comment(line):
    ...
    though they were code — prose about a banned pattern is not a use of it.
    """
    quote = None
    i = 0
    while i < len(line):
...
class TestSaveReturnChecked:
    ...
    write failed (#74-#77). The full .save() sweep is complete (cfg.save S5 +
    meshcore config.save S8 M1/M2 + the automation/first_run settings.save sweep) —
...
class TestConfigPathContract:
    ...
    It collected violations and then did nothing with them — no assert, and
    despite its comment, no print either. It could not fail and did not
    inform, but it counted as a guard and read as coverage. A drill planting
    ``Path('/home/pi/.reticulum/config')`` in src/ passed, and always would
    have; unlike the rest of this file that was never the SRC_DIR bug, it was
    written that way.

    Its premise doesn't survive contact with the tree: the pattern matches 38
    places here, nearly all user-facing strings and docstrings telling an
    operator which file to edit. There is no threshold separating those from
    a real hardcode with a line scanner.

    What actually enforces this contract: lint MF009 / MF019, ``ReticulumPaths``
    as the path SSOT, and the sibling test below — all of which have teeth.
    """
    ...
    def test_gateway_rns_clients_use_shared_configdir(self):
        ...
        (gw-resourcepath-determinism, ported from MeshForge 2026-06-27). A new
        inline ``/tmp/meshanchor_rns_client`` builder in either file would
        reintroduce the race the helper fixes."""
        for rel in ("gateway/_rns_bridge_connection.py",
            ...
            assert 'tempfile.gettempdir()) / "meshanchor_rns_client"' not in src, (
...
class TestServingNeverBlocksOnCollection:
    ...
    block unboundedly on an external source. The meshtastic interface
    constructor blocks until the full nodedb sync; a wedged daemon can stall it
    ~900s while the map collector holds _collect_lock, wedging every
    /api/nodes/geojson + /api/network/topology request behind it.
    ...
    def test_interface_create_only_in_bounded_helper(self):
        ...
        assert not offenders, (
            ...
            f"again (the 2026-06-23 MeshForge moc1 spin). Offenders "
            f"(line, call, fns): {offenders}"
        )
    ...
    def test_direct_radio_gated_on_meshtasticd_presence(self):
        ...
        (service state), not on `not tcp_features` alone — else a wedged
        meshtasticd (empty result, daemon present) lets the fallback seize
        /dev/ttyACM0, a DIFFERENT radio (dude-claw's)."""
        tree = self._parse(self.ORCHESTRATOR)
        guarded = False
        for node in ast.walk(tree):
            if not isinstance(node, ast.If):
                continue
            # THE gate is uniquely the `if` whose TEST references tcp_features
            # (the `not tcp_features and ...` guard) AND whose body triggers the
            # direct-radio collect — not the outer `if self._meshtastic_enabled:`
            # wrapper, which also contains the call deeper in its body.
            test_dump = ast.dump(node.test)
            if 'tcp_features' not in test_dump:
                continue
            body_dump = " ".join(ast.dump(s) for s in node.body)
            if '_collect_direct_radio' not in body_dump:
                continue
            assert '_meshtasticd_present' in test_dump, (
                ...
                "seizure (ttyACM0 cascade, MeshForge 2026-06-23)."
            )
            guarded = True
            break
        assert guarded, (
...
class TestDetectorHonesty:
    ...
    this shape. (Ported from MeshForge 2026-06-23; MeshAnchor has no dude-claw
    host_probe witness, so that verdict guard is MeshForge-only.)
    """
    ...
    def _collector(self):
        ...
        return MapDataCollector(cache_dir=d, enable_history=False)
...
class TestConfigAtomicityMF026:
    """MF026: config/state persistence must be atomic (ported from MeshForge).
    O_TRUNC is banned; new non-atomic config open('w') writes fail against a
    frozen baseline. Route through utils.paths.atomic_write_text.
    """

    _REPO_ROOT = os.path.abspath(os.path.join(SRC_DIR, '..'))
    ...
    def test_no_config_atomicity_violations_above_baseline(self):
        ...
        issues = lint_mod.check_config_atomicity(files, self._REPO_ROOT)
    ...
    def test_baseline_only_shrinks(self):
        lint_mod = self._lint()
        assert sum(lint_mod.MF026_BASELINE.values()) <= 7
...
class TestPrivilegedPycachePrefix:
    ...
    def test_installed_commands_are_symlinks_not_generated_copies(self):
        ...
        for rel in self.INSTALLERS:
            ...
            for i, l in enumerate(flines, 1):
                ...
                dest = r'/usr/local/bin/meshanchor(?:-tui|-noc|-lora|-status|-web|-map)?(?![-\w])'
        ...
        for dest in ('meshanchor', 'meshanchor-noc', 'meshanchor-lora', 'meshanchor-status',
                     'meshanchor-web', 'meshanchor-map'):
            want = f'ln -sfn /opt/meshanchor/scripts/meshanchor-launcher.sh /usr/local/bin/{dest}'
        ...
        launcher = os.path.join(self.REPO, 'scripts/meshanchor-launcher.sh')
        with open(launcher, 'r', encoding='utf-8') as fh:
            ltext = fh.read()
        for name, sub in (('meshanchor-tui', 'tui'), ('meshanchor-noc', 'noc'),
                          ('meshanchor-lora', 'lora'), ('meshanchor-status', 'status'),
                          ('meshanchor-web', 'web'), ('meshanchor-map', 'mapserver')):
            assert re.search(r'^\s*' + re.escape(name) + r'\)\s+set -- ' + sub + r' "\$@"',
                             ltext, re.M), (
                f"scripts/meshanchor-launcher.sh has no basename dispatch for {name} "
    ...
    def test_launchers_source_the_shared_constant(self):
        ...
        path = os.path.join(self.REPO, 'scripts/meshanchor-launcher.sh')
        with open(path, 'r', encoding='utf-8') as fh:
            text = fh.read()
        assert 'lib/pycache_prefix.sh' in text, (
            "scripts/meshanchor-launcher.sh defines the pycache path itself "
            "instead of sourcing scripts/lib/pycache_prefix.sh")

        path = os.path.join(self.REPO, 'scripts/meshanchor-terminal.sh')
        with open(path, 'r', encoding='utf-8') as fh:
            lines = [l.strip() for l in fh.read().splitlines()]
        want = 'TUI_CMD="$MESHANCHOR_DIR/scripts/meshanchor-launcher.sh tui"'
        assert want in lines, (
            "scripts/meshanchor-terminal.sh must delegate to the launcher script, "
        ...
        assert own_launch == [], (
            "scripts/meshanchor-terminal.sh builds its own launch command instead of "
