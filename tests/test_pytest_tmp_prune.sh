...
# (MF review port 2026-07-28): a second, runtime python3+_pytest derivation on
# every cron run guarded the same constant a third time, asked bare python3
# rather than the venv consumer-of-record, and cost an interpreter spawn per
# run on every box for an answer that changes ~never.
...
# masked by "the wrapper crashed" when it ran fine (MF review port 2026-07-28).
