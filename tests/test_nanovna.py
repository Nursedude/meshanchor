...
class TestHandlerSurface:
    ...
    def test_menu_tag_is_in_the_section_ordering(self):
        ...
        MeshAnchor keeps its orderings inline per menu method (MeshAnchor
        centralised them into SECTION_ORDERINGS in its Q5 arc), so this reads
        the source rather than a dict.
        """
        import pathlib
        main_py = pathlib.Path(__file__).resolve().parents[1] / 'src' / 'launcher_tui' / 'main.py'
        text = main_py.read_text()
        start = text.index('def _rf_sdr_menu')
        ordering_line = text[start:start + 600]
        assert '"vna"' in ordering_line, "vna missing from the rf_sdr ordering"
