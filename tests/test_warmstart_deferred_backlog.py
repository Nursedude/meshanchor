...
def test_due_TODAY_is_surfaced_like_the_watcher_pages_it(tmp_path):
    """Boundary pin against the ledger's OWNING consumer — MeshForge's
    scripts/deferred_work_watch.py on the shared manager box (the ledger is
    operator-level, one per fleet; MA carries no watcher of its own): it
    pages when today >= review_after (`today < ra_date -> continue`). The
    first cut here used `ra < today`, so the task the operator was paged
    about this morning was invisible in the same morning's warm-start
    brief."""
