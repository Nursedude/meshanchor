...
~30-40 min before its next run, so the stale-cron detector read
...
class TestCronVerdictConcurrentWriters(unittest.TestCase):
    ...
    then VANISHES while the cron demonstrably ran (the 2026-07-16 field shape,
    ported from MeshForge). Every concurrent verdict must survive.
    """
