"""Tests for updates.meshanchor_git — the git-layer SSOT for MeshAnchor
...
import updates.meshanchor_git as mg
...
class TestGetState:
    def test_not_a_repo(self, tmp_path):
        state = get_meshanchor_git_state(repo_dir=tmp_path)
    ...
    def test_behind_flags_update(self, tmp_path):
        ...
        with patch.object(mg, "_git", side_effect=_git_router(responses)):
            state = get_meshanchor_git_state(repo_dir=repo)
...
class TestRunUpdate:
    ...
    def test_verified_fast_forward(self, tmp_path):
        ...
        with patch.object(mg, "get_meshanchor_git_state",
                          side_effect=[self._state(), after]), \
                patch.object(mg, "_git", return_value=_cp(0, "Fast-forward")):
            ok, detail = run_meshanchor_git_update(repo_dir=repo)
        assert ok is True
...
class TestServicesToRestart:
    def test_parses_active_units(self):
        out = ("meshanchor.service loaded active running MeshAnchor NOC\n"
               "meshanchor-map.service loaded active running MeshAnchor Map\n")
        with patch.object(mg.subprocess, "run", return_value=_cp(0, out)):
            assert meshanchor_services_to_restart() == [
                "meshanchor", "meshanchor-map"]
