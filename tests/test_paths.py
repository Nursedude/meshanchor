...
class TestReticulumClientConfigdir:
    ...
    deterministic (gw-resourcepath-determinism, ported from MeshForge
    2026-06-27). The FIXED location + NO-interface config are the two
    ...
    def test_returns_canonical_meshanchor_rns_client(self, tmp_path):
        d = self._call(tmp_path)
        assert d == os.path.join(str(tmp_path), "meshanchor_rns_client")
    ...
    def test_writes_no_interface_client_config(self, tmp_path):
        ...
        # Invariant: NO interface section ([[...]]) — would bind ports rnsd owns.
        assert "[[" not in cfg
