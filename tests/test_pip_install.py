...
Ported from MeshForge (install-hardening parity). Covers the root-cause fix for
the fresh-user failure (no pip-presence guard) and the silent-failure closes:
ensure_pip ladder, return-code checking, conflict-retry firing exactly once,
PEP 668 override precedence, and import-as-consumer verification flipping a
"successful" install to not-ok.
"""
...
def test_pep668_active_false_in_venv():
    ...
    with patch("utils.pip_install.subprocess.run", rec):
        assert pep668_active("/opt/meshanchor/venv/bin/python") is False
...
def test_resolve_target_python_falls_back_to_python3():
    with patch("updates.version_checker.get_meshanchor_venv_dir", return_value=None):
        assert resolve_target_python() == "python3"


def test_resolve_target_python_uses_venv():
    with patch("updates.version_checker.get_meshanchor_venv_dir",
               return_value="/opt/meshanchor/venv"):
