...
# swallow, and we only print ✓ after a real success.
...
# Optional source pin: MESHANCHOR_REF=<tag|branch|sha> installs that exact ref.
...
if [[ -d "$INSTALL_DIR/.git" ]]; then
    ...
    if [[ -n "$MESHANCHOR_REF" ]]; then
        git clone -q --branch "$MESHANCHOR_REF" https://github.com/Nursedude/meshanchor.git "$INSTALL_DIR" 2>/dev/null \
...
if [[ -n "$MESHANCHOR_REF" ]]; then
    ...
    git checkout -q "$MESHANCHOR_REF" \
        || { echo -e "${RED}  ✗ MESHANCHOR_REF '$MESHANCHOR_REF' could not be resolved${NC}" >&2; exit 1; }
fi
...
# source these before the clone). Adds ensure_pip + checked pip + transcript.
...
# Check for PEP 668 via the shared SSOT detector - Debian Bookworm, RPi OS
