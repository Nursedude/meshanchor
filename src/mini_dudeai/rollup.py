...
larger claim one `cat` cannot make. Both twins' conventions are tried, so a
MeshForge box's daemon shows up here as a healthy FOREIGN box rather than an
absent one.

Ported from MeshForge 2026-08-12, where this cost 19 days: MF's pane read THIS
repo's replica as "runs no mini" from our 07-24 move off the home-dir
convention onward, while the daemon ticked every 30s. The mirror was LATENT on
this side only because MA's fleet_hosts lists one box — adding any MeshForge
box would have reproduced it exactly. A fix applied to one instance is not
applied to the class.

A host that won't answer ssh is ``unreachable``.
"""
...
#: that dir is "" and these equal the basenames; here it is the MA subdir.
...
def collect_remote(host: str, now_ts: float, timeout_s: float = DEFAULT_SSH_TIMEOUT_S,
    ...
    make — the one that hid a healthy MeshAnchor daemon from MeshForge's
    pane for 19 days (2026-08-12).
    """
...
def build_rollup(postures: list[dict], now_ts: float) -> str:
    ...
    for p in ordered:
        ...
        if p["status"] == "no_state_file":
            ...
            # measured; on the MF twin it read as expected furniture for 19 days.
...
def build_box_deep(host: str, state: dict, history: list[dict], now_ts: float,
    ...
    # defect the brief was fixed for and this sibling was not (found in
    # MeshForge 2026-08-11, ported here the same day: a fix applied to one
    # instance is not applied to the class).
