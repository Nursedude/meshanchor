"""NomadNet launch-error / log-viewer / config-file IO operations.

Code-motion extraction from ``nomadnet.py`` (1,500-line rule), mirroring
MeshForge's ``_nomadnet_io_ops.py`` boundary: stderr-based launch-error
diagnosis, the logfile viewer, and view/edit of the NomadNet config file.
Mixed back into ``NomadNetHandler`` in ``handlers/nomadnet.py`` (the hub)
— zero behavior change, no external import paths affected.
"""
...
class NomadNetIOOpsMixin:
    """Launch-error display, log viewer, and config view/edit for NomadNet.

    Host class must provide ``self.ctx`` (TUIContext) plus
    ``_get_nomadnet_config_path``, ``_find_nomadnet_binary`` and
    ``_get_rns_config_for_user`` (all defined on ``NomadNetHandler`` or
    its other mixins).
    """
