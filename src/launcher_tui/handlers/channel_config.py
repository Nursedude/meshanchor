...
Edits the eight Meshtastic channel slots (PRIMARY + 7 SECONDARY) by
shelling out to the meshtastic CLI through ``commands.meshtastic``.
Surfaces channel name / PSK / role and offers gateway-channel templates.

Ported from MeshForge 2026-05-04 (MN-1a) — the only file in MN-1's
original six-file scope that was self-contained. The other five depend
on ``core/meshtasticd_config`` + ``core/meshtastic_cli`` + ``utils/
meshtastic_http`` (~2,600 LOC of infrastructure not in MeshAnchor); see
the parity tracker for the deferred MN-1b decision.

Surgical adjustments from MeshForge original:
  - ``feature_flag = "meshtastic"`` on the ``channels`` row so the
    handler hides under MESHCORE / RADIO_MAPS / MONITOR profiles
    (Phase 3 audit policy).
  - User-facing brand strings "MeshForge" → "MeshAnchor" in the primary
    channel default placeholder and the gateway-channel explainer.
"""
...
class ChannelConfigHandler(BaseHandler):
    ...
    def menu_items(self):
        return [
            ("channels", "Channel Config      Meshtastic channels", "meshtastic"),
        ]
        ...
        try:
            ...
            if result.success:
                ...
                if method == 'usb':
                    ...
                    msg = "Connected via TCP: localhost:4403"
    ...
    def _set_gateway_channel(self):
        ...
        confirm = self.ctx.dialog.yesno(
            ...
            "MeshAnchor <-> RNS gateway bridging.\n\n"
