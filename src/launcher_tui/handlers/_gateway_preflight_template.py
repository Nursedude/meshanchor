...
def check_template_drift_best(
    ...
    A fleet may deliberately run more than one radio profile (MeshForge's is
    LONG_FAST/slot20 + SHORT_TURBO/slot8 joined by a cross-preset bridge —
    operator decision, 2026-07-09), so a single default template structurally
    false-fails whichever leg it wasn't exported from. Each box is judged
    against the template it matches best (fewest fails, then fewest warns);
    a box matching NO template still reports the closest one's failures
    loudly. Ported from MeshForge 2026-07-09.
...
def _check_one(
    ...
    # radio query or an absent gateway.json leaves live values missing, and
    # comparing them against an expectation manufactures FAILs on every
    # non-gateway box (MeshForge fleet sweep, 2026-07-09; ported same day).
    # KEY-presence, not value-None: capture_live_state omits keys it could
    # not observe, but stores an explicit None when the source was READ and
    # the field was missing — that is OBSERVED drift and keeps failing.
    # Categories with their own branches (packages/services/
    # rns_shared_instance/nomadnet) always OBSERVE their values.
