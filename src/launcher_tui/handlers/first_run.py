"""
First-Run Handler — Initial setup wizard for new MeshAnchor installations.

Converted from first_run_mixin.py as part of the mixin-to-registry migration (Batch 8).

Guides new users through MeshAnchor setup:
...
class FirstRunHandler(BaseHandler):
    ...
    FIRST_RUN_FLAG = ".meshanchor_setup_complete"
    ...
    def _run_first_run_wizard(self) -> bool:
        ...
        result = self.ctx.dialog.yesno(
            "Welcome to MeshAnchor!",
            "It looks like this is your first time running MeshAnchor.\n\n"
            ...
            if self._is_raspberry_pi():
                if self._offer_enable_spi():
                    ...
                    self.ctx.dialog.msgbox(
                        ...
                        "  sudo raspi-config\n\n"
                        "Or from System > Hardware in MeshAnchor."
                    )
                    return
            else:
                self.ctx.dialog.msgbox(
...
# Generated by MeshAnchor Setup Wizard
#
...
"""
    ...
    def _wizard_step_hardware(self):
        ...
        if results['meshtastic_candidates']:
            ...
            lines.append("\nTo use MeshAnchor with a radio:")
        ...
        if result:
            ...
            if callsign:
                ...
                try:
                    ...
                    settings = SettingsManager("meshanchor")
