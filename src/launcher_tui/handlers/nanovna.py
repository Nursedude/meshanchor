...
``handlers/extensions.py`` has zero references to ``plugins/``. So MeshAnchor
...
class NanoVNAHandler(BaseHandler):
    ...
    def _baseline_dir(self):
        ...
        return get_real_user_home() / ".local" / "share" / "meshanchor" / "vna"
    ...
    def _vna_menu(self):
        ...
        while True:
            ...
            if entry:
                ...
                self.ctx.dialog.msgbox(
                    "Not wired",
                    f"No action owns '{choice}' — MeshAnchor wiring bug.")
    ...
    def _manage_baselines(self):
        while True:
            ...
            pick = self.ctx.dialog.menu(
                "Saved Baselines",
                f"{len(paths)} saved in ~/.local/share/meshanchor/vna/\n"
