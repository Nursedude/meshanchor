...
BRIDGE-LEG axis is NOT auto-applied: the leg is the gateway.json ``bridge_mode``
(MeshAnchor is MeshCore-primary and runs the bridge inside meshanchor-daemon), set
per-box in ``~/.config/meshanchor/gateway.json``; it is surfaced as a guided next
step. Local box only (no SSH/remote apply).

Ported from MeshForge alongside the provisioner. Pure logic lives in
...
class FleetProvisionHandler(BaseHandler):
    ...
    def _show_current(self, mod) -> None:
        ...
        self.ctx.dialog.msgbox("Current box", "\n".join(lines))
    ...
    def _preview(self, mod, preset_name: str, doc) -> None:
        ...
        try:
            ...
            self.ctx.dialog.msgbox(f"Preview: {preset_name}", "\n".join(lines))
            return
        ...
        if os.geteuid() == 0:
            lines += ["",
                      ...
                      "ROLE (you confirm the exact changes first). The bridge leg "
                      "is set separately via the gateway.json bridge_mode."]
        else:
            lines += ["",
