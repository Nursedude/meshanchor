...
DEFAULT_ROOT = "/opt/meshanchor"

# plan() verbs that mean "the box would actually change under converge".
CHANGE_VERBS = ("enable", "disable", "mask")

# The guided next-step for the BRIDGE-LEG axis. apply_preset converges the ROLE
# axis only; the bridge leg is the gateway.json `bridge_mode`, applied per-box.
# MeshAnchor has no configure_gateway.sh (unlike MeshForge) — the bridge runs
# inside meshanchor-daemon, so the leg is set in the gateway.json config directly.
CONFIGURE_GATEWAY_CMD = (
    "set bridge_mode in ~/.config/meshanchor/gateway.json, "
    "then restart meshanchor-daemon")
