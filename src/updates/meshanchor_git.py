"""git-layer SSOT for MeshAnchor self-update state.

Born from the 2026-07-10 follow-up ("I have not successfully updated
MeshAnchor"): the old self-update compared version STRINGS (src/__version__.py
...
@dataclass
class MeshAnchorGitState:
    """Re-derived git truth for the MeshAnchor checkout."""
...
def get_meshanchor_git_state(fetch: bool = True,
                            branch: str = DEFAULT_BRANCH,
                            repo_dir: Optional[Path] = None) -> MeshAnchorGitState:
...
def run_meshanchor_git_update(branch: str = DEFAULT_BRANCH,
    ...
    before = get_meshanchor_git_state(fetch=True, branch=branch, repo_dir=repo)
    ...
    after = get_meshanchor_git_state(fetch=False, branch=branch, repo_dir=repo)
...
def meshanchor_services_to_restart() -> List[str]:
    """Active meshanchor-owned systemd units that host the updated code.
