...
``MESHANCHOR_ORACLE_ENABLED`` (see ``from_env``).
"""
...
class MeshOracleResponder:
    ...
    def from_env(
        ...
        *,
        ...
        - ``MESHANCHOR_ORACLE_ENABLED``: 1/true/yes/on to enable (else ``None``);
          shared across legs.
        - ``allowlist_env`` (default ``MESHANCHOR_ORACLE_ALLOWLIST``; the RNS leg
          passes ``MESHANCHOR_ORACLE_RNS_ALLOWLIST``): comma sender-ids, or ``"*"``
        ...
        - ``MESHANCHOR_ORACLE_COOLDOWN_S``: per-sender min seconds (default 30).
        - ``MESHANCHOR_ORACLE_CONSUME``: 1/true (default) = a handled query is
        ...
        if str(env.get("MESHANCHOR_ORACLE_ENABLED", "")).strip().lower() not in _TRUE:
            return None
