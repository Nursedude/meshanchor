...
WATCHDOG_STATE_PATH = Path("/var/lib/meshanchor/watchdog.json")
...
class NocSnapshot:
    ...
    # watchdog (/var/lib/meshanchor/watchdog.json)
...
def oracle_log_path() -> Path:
    ...
    Lives under the MeshAnchor DATA dir (``~/.local/share/meshanchor``), which the
    daemon unit carries in ``ReadWritePaths`` (it does NOT set ``ProtectHome``) —
    so the write succeeds. (This is also the right home defensively: the sibling
    MeshForge gateway runs ``ProtectHome=read-only`` and a write to bare ``~/``
    silently failed there — #60.) Pure: returns a Path; the caller does the write.
    """
    from utils.paths import MeshAnchorPaths
    return MeshAnchorPaths.get_data_dir() / "mesh_oracle_log.jsonl"
