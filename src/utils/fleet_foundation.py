...
Ratified at the 2026-06-01 architecture meeting (MeshForge is the lead repo for
this arc: `.claude/plans/fleet_permission_foundation_plan.md`). One definition of
the permission foundation a box must have so its services run as the non-root
...
- **RNS foundation** — the `/etc/reticulum` tree perms + the rnsd service-user.
  ...
  (``scripts/parity_check.py`` in MeshForge gates both). This module *delegates*
  to it, never re-implements it.
- **App data-roots** — this app's own data dirs (``~/.config/meshanchor`` etc.).
  These differ per app *by design*, so they are NOT byte-parity-tracked. The
  audit/plan **engine** below is parity-shared (pure functions over an injected
  owner-lookup, shape-tier identical to MeshForge); only
  ``meshanchor_data_roots()`` is MeshAnchor-specific.
...
# --- app-specific declaration (MeshAnchor) — MeshForge ships its own ----------


def meshanchor_data_roots(home: Optional[Path] = None) -> Tuple[Path, ...]:
    """MeshAnchor's persistent data dirs that a non-root service must own.

    The roots proven by the meshanchor-server convergence (2026-06-01): the
    XDG-style app dirs plus the install dir (the daemon writes logs/db under
    /opt/meshanchor). The RNS tree (/etc/reticulum) is handled separately by
    rns_tree_perms (the shared parity core), not listed here. ``~/.cache`` was the
    live-caught miss on the first converge pass — the data-root list must be
    COMPLETE or a non-root service hits Permission denied at runtime.
    """
    home = home or get_real_user_home()
    return (
        home / ".config" / "meshanchor",
        home / ".local" / "share" / "meshanchor",
        home / ".cache" / "meshanchor",
        Path("/opt/meshanchor"),
    )
...
def canonical_foundation(
    ...
    return FoundationSpec(
        operator_user=user,
        data_roots=meshanchor_data_roots(home),
    )


# --- pure engine (parity-shared with MeshForge: no app-specific paths in here) -
...
def audit_data_roots(
    ...
    this is the parity-shared core and is unit-testable without a real fs.
    """
    reasons: List[str] = []
...
def plan_foundation(spec: Optional[FoundationSpec] = None) -> List[FoundationAction]:
    ...
    shared ``rns_tree_perms`` apply-path).
    """
    spec = spec or canonical_foundation()
