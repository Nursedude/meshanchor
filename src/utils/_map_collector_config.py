...
Extracted from map_data_collector.py 2026-07-14 for MF025 size compliance (the MA
twin of MeshForge's MapCollectorConfigMixin split). Pure code motion — no behavior
change. These methods only touch ``self._settings``, the ``self.DEFAULT_*`` class
attributes (defined on MapDataCollector), and stdlib; they carry NO hub-global
dependencies, so no ``_hub()`` indirection is needed.
...
# clock skew / a hostile injected stamp) and must not read as "online". Consumed
# by _is_node_online.
