"""Unified, hardened Python package installer for MeshAnchor.

THE one place pip is invoked from Python code. Ported from MeshForge's
src/utils/pip_install.py (install-hardening parity, 2026-06-23). Born from a
recurring failure class (feedback_version_env_rigor, feedback_install_method_fragility):
a fresh user's app+env "did not install properly" and they had to hand-install
pip, because nothing in the running app checked that pip even EXISTS before
calling it — no ensurepip, no apt fallback, no actionable error. On top of that,
~10 pip-invocation sites each re-implemented PEP 668 handling and return-code
checking with varying fidelity (and several hardcoded the venv path, ignoring
`.no-venv`), and some swallowed failures into a valid-looking success.
...
import from installer, TUI handlers, and tests alike. `get_meshanchor_venv_dir`
...
def resolve_target_python(explicit: Optional[PathLike] = None) -> str:
    """SSOT for 'which interpreter does MeshAnchor install into'.

    Precedence: an explicit target (the rnsd-python case, a venv path, or
    ``sys.executable``) wins; else MeshAnchor's venv python if the updater would
    use it (`get_meshanchor_venv_dir`); else system ``python3``. Returning the
    ...
    if explicit:
        return str(explicit)
    try:
        from updates.version_checker import get_meshanchor_venv_dir
        venv_dir = get_meshanchor_venv_dir()
    except Exception as exc:  # pragma: no cover - defensive
        _log.debug("get_meshanchor_venv_dir unavailable: %s", exc)
        venv_dir = None
...
def pep668_active(python: Optional[PathLike] = None) -> bool:
    ...
    the call sites stop hard-coding ``--break-system-packages``. A venv is NEVER
    externally managed (pip exempts it regardless of any marker in the shared
    stdlib), so we answer False inside a venv and only True for a base
    interpreter that actually carries the EXTERNALLY-MANAGED marker.
    """
...
def check_import(
    ...
    *,
    ...
    Reuses the gateway_diagnostic import-check shape. ``sudo``/``sudo_user`` run
    the check as the principal that will actually consume the package (root for
    rnsd, the gateway user for libs), because "installed" and "importable by the
    consumer" are different facts (Issue #24).
    """
    py = str(python)
    ...
    *,
    ...
    Steps:
      ...
      (e) on a conflict signature, retries exactly once with
          ``--ignore-installed`` (the pattern previously duplicated per-site);
...
def pipx_install(
    ...
    *,
    ...
    stay in the caller so we don't regress the read/write-split fix. Pass an
    already-resolved ``owner_uid`` to run pipx as that user; otherwise it runs as
