...
MeshForge-parity port (2026-07-03). Consumers compare an installed package
version against the FLEET BASELINE declared in ``requirements/*.txt``:

* ``utils.active_health_probe.ActiveHealthProbe.check_dep_version_floor`` —
  flags this box's own interpreter env BELOW the floor (a missed/failed
  update; the class that left meshanchor-server silently on meshtastic 2.7.8
  while the fleet pin was 2.7.9, found 2026-07-03).
* Any future TUI update checker (MeshForge's ``updates.version_checker`` is
  the template) must import THIS parser + comparison, not grow its own.

If two consumers parsed the requirements file with even slightly different
semantics they would disagree about what "the floor" is — exactly the
two-constants-drift trap honest_failure_modes item 5 warns about. This module
is the ONE parser + ONE comparison, byte-compatible with MeshForge's copy.
"""
...
def version_below(have, floor) -> bool:
    ...
    for both a health page and an "update available" badge).
    """
