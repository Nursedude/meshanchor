"""
MeshAnchor MQTT Broker Profiles

Provides broker configuration templates following Meshtastic MQTT conventions:
- MeshAnchor Private Broker (localhost mosquitto, custom PSK)
...
MeshAnchor to act as its own MQTT broker for bridging:
...
class BrokerType(Enum):
    ...
    PRIVATE = "private"    # MeshAnchor-managed mosquitto
...
def create_private_profile(
    ...
    channel: str = "LongFast",
    region: str = "US",
    username: str = "meshanchor",
    password: str = "",
) -> BrokerProfile:
    """Create a MeshAnchor private broker profile.
    ...
    return BrokerProfile(
        ...
        acl_enabled=True,
        description=(
            "MeshAnchor private broker (localhost mosquitto). "
...
# MeshAnchor Private MQTT Broker Configuration
# Generated by MeshAnchor broker profile manager
#
...
# Plain TCP listener for local Meshtastic devices and MeshAnchor
listener {port}
protocol mqtt
...
# =============================================================================
# MeshAnchor user - full access to mesh topics
# =============================================================================
user {username}
topic readwrite msh/#
topic readwrite meshanchor/#
topic read $SYS/#
...
def generate_mosquitto_conf(profile: BrokerProfile) -> str:
    ...
    if profile.allow_anonymous:
        ...
        password_file_line = "password_file /etc/mosquitto/meshanchor_passwd"
        acl_file_line = "acl_file /etc/mosquitto/meshanchor_acl"
...
def generate_mosquitto_acl(profile: BrokerProfile) -> str:
    ...
    return MOSQUITTO_ACL_TEMPLATE.format(
        username=profile.username or "meshanchor",
...
def _get_profiles_path() -> Path:
    ...
    return get_real_user_home() / ".config" / "meshanchor" / PROFILES_FILENAME
...
def install_mosquitto_config(profile: BrokerProfile) -> Tuple[bool, str]:
    ...
    - /etc/mosquitto/meshanchor_passwd (password file)
    - /etc/mosquitto/meshanchor_acl (ACL file)
    ...
    try:
        ...
        conf_path = conf_dir / "meshanchor.conf"
        ...
        if not profile.allow_anonymous and profile.username and profile.password:
            passwd_path = Path("/etc/mosquitto/meshanchor_passwd")
        ...
        if profile.acl_enabled and not profile.allow_anonymous:
            acl_path = Path("/etc/mosquitto/meshanchor_acl")
...
def ensure_default_profiles() -> Dict[str, BrokerProfile]:
    ...
    if not profiles:
        profiles["meshanchor_private"] = create_private_profile()
