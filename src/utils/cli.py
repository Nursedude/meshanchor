...
def diagnose_meshtastic_cli(cli_path=None):
    ...
    The 2026-07-10 MeshForge audit found a box where ``~/.local/bin/
    meshtastic`` was a pip ``--user`` console script (system-python shebang)
    SHADOWING the pipx-managed shim — so ``pipx upgrade`` rewrote a venv the
    wrapper never executes and the running CLI silently stayed put (the
    read/write split at the binary level). This diagnosis lets the updater
    target the writer that actually owns what runs.
