...
class RNSStatus:
    ...
    # rns_rpc_unresponsive probe keys on this flag.
    timed_out: bool = False
    # Wall time the rnstatus subprocess actually took, by MONOTONIC clock
    # (wall-clock durations are forgeable on RTC-less Pis). None when no
    # subprocess ran — never 0.0, which would be indistinguishable from an
    # impossibly fast run and would poison a latency baseline built from
    # these values. On a TIMEOUT this is the bound we gave up at, not a
    # measurement of rnsd.
...
def run_rnstatus(timeout_s: float = 15.0) -> RNSStatus:
    ...
    Args:
        timeout_s: subprocess timeout. Default 15s preserves existing
            callers (e.g. rns_monitor handler). The watchdog passes a
            shorter value so a wedged rnsd can't stall its tick.
    ...
    try:
        ...
        # (falsifiability audit phase 2, 2026-09-02; MeshForge lead, ported).
