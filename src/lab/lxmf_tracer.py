...
Reads peers from ``~/.config/meshanchor/lab_peers`` (``name=hash`` per
...
def _peers_file_default() -> Path:
    ...
    return get_real_user_home() / ".config" / "meshanchor" / "lab_peers"
...
def _build_client_config(tmpdir: Path) -> Path:
    ...
    lines = [
        "# MeshAnchor lab-tracer RNS client config (auto-generated)",
        "[reticulum]",
        "share_instance = Yes",
...
def run_trace(
    ...
    with tempfile.TemporaryDirectory(prefix="meshanchor_lab_tracer_") as tmp:
        tmpdir = Path(tmp)
...
def main(argv=None) -> int:
    ...
    parser.add_argument(
        ...
        help="Override path to lab_peers file "
             "(default ~/.config/meshanchor/lab_peers)",
    )
    parser.add_argument(
