...
Identity: ``~/.config/meshanchor/lab_echo_identity`` (auto-generated on
first run). The destination hash is logged at startup so an operator
can ssh-distribute it into peers' ``~/.config/meshanchor/lab_peers``.
...
def _build_client_config(tmpdir: Path) -> Path:
    ...
    lines = [
        "# MeshAnchor lab-echo RNS client config (auto-generated)",
        "[reticulum]",
        "share_instance = Yes",
...
def run_daemon(loglevel: str = "INFO", announce_interval_s: int = ANNOUNCE_INTERVAL_S) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="meshanchor_lab_echo_") as tmp:
        tmpdir = Path(tmp)
