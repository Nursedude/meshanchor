...
class PersistentMessageQueue(MessageQueueLifecycleMixin):
    ...
    # retry_after is a wall-clock schedule; on RTC-less Pis a large BACKWARD
    # NTP step after scheduling would strand a message far in the future
    # (Pri-9, gateway review 2026-07-23). No legitimate backoff schedules a
    # retry more than this far out, so get_pending releases anything beyond the
    # ceiling — a clock-artifact release (honest_failure_modes #6 absurd-delta).
    ...
    def __init__(self, db_path: Optional[str] = None,
        ...
        # message fired ~45ms apart lost chunks 2-3 silently (MeshForge,
        # 2026-06-04). Pacing the dispatch loop cures the whole burst class.
    ...
    def _get_connection(self):
        ...
        # + 30s busy_timeout. Phase 1 of post-fleet-host-2026-04-26 closure
        # — was missing sync=NORMAL and journal_size_limit before.
        conn = connect_tuned(self._db_path)
        conn.row_factory = sqlite3.Row
        # _wal_initialized stays around for compat with any callers reading
        # it; the helper applies WAL on every connect (idempotent on header).
    ...
    def register_pending_ack(
        ...
        Args:
            ...
            allow_orphan: when True and the message_id has no queue row
                (e.g. LXMFBroadcastBridge does its own LXMF send and
                doesn't go through enqueue()), INSERT a synthetic
                ...
                via UPDATE of an existing row or INSERT+UPDATE of a
                synthetic one), False only on hard DB error.
        ...
        with self._get_connection() as conn:
            ...
            """, (
                ...
                None,
                "",
                message_id,  # content_hash = unique msg_id avoids dedup collisions
                1,
                'pending',
                timeout_at,
                origin_network,
    ...
    def _shed_overflow(self, count: int = 1) -> int:
        ...
        # Each shed message was recorded QUEUED at enqueue — without a
        # terminal DROPPED here its lifecycle reads QUEUED-then-nothing
        ...
        # recording site until 2026-07-09 (frontier review Pri-2, ported).
