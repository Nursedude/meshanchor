...
class UnifiedNode:
    ...
    # placeholder overwrite a good one. Deliberately NOT persisted — every
    # announce builds a fresh node object, so a cached value would be
    # meaningless. Ported from MeshForge 2026-07-21.
    ...
    def from_rns(cls, rns_hash: bytes, name: str = "", app_data: bytes = None,
        ...
        if RNS_SERVICES_AVAILABLE and service_info is not None:
            ...
            if service_info.display_name:
                ...
                # name recorded by an older, buggier parser.
