...
class PKIStatus:
    ...
    def from_dict(cls, d: dict) -> "PKIStatus":
        ...
        Twin of the MeshForge 2026-07-21 review finding (C1): to_dict() wrote
        pki_status but no loader ever restored it — every restart erased
        every node's key baseline, so the 'PKI KEY CHANGED — potential MITM'
        branch could never fire across a restart (the key was silently
        re-TOFU'd as first-seen). Honest guard: no recoverable key material →
        UNKNOWN (no baseline), never a TRUSTED-with-None-key state that would
        false-MITM the next observation.
        """
...
class UnifiedNode:
    ...
    # placeholder-grade provenance. Ported from MeshForge.
    ...
    def to_dict(self, include_signal_history: bool = False) -> dict:
        ...
        result = {
            ...
            # so cached MeshCore nodes lost their pubkey/role on restart)
    ...
    def from_meshtastic(cls, mesh_node: dict, is_local: bool = False) -> 'UnifiedNode':
        ...
        node = cls(
            ...
            # a rename can correct a stale cached name (MeshForge 2026-07-21
            # review W1: the earlier fix only covered the RNS leg; a renamed
            # Meshtastic node was mis-named in the cache forever). The
            # meshtastic_id fallback is a placeholder and must never displace
            # a good name.
    ...
    def from_meshcore(cls, advertisement: Any, **kwargs) -> 'UnifiedNode':
        ...
        # contact list from get_contacts()). Hard-coding now()/is_online=True
        # marked every swept contact "online / 0s ago" the moment a
        # contact-sweep caller existed — finding-C's defect class
        # (honest_failure_modes #1). Matters more here (MeshCore-primary); no
        # production caller sweeps contacts today, so this closes it before one does.
        if isinstance(advertisement, dict):
    ...
    def from_rns(cls, rns_hash: bytes, name: str = "", app_data: bytes = None,
        ...
        if RNS_SERVICES_AVAILABLE and service_info is not None:
            ...
            if service_info.display_name:
                ...
                # The node named itself — outranks a cached placeholder or a
                # name recorded by an older, buggier parser. BUT only a
                # STRUCTURED parse earns that authority: a heuristic byte-scan
                # "name" (the mojibake mechanism) may fill an empty slot,
                # never correct a good one (MeshForge 2026-07-21 review W2).
