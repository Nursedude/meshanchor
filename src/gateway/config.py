...
# (1,500-line rule code-motion; this module stays the import hub, so
# every name below remains importable from gateway.config unchanged.)
# =============================================================================

from .config_validators import (  # noqa: F401 — re-exports
...
class MeshtasticConfig:
    ...
    # MeshForge Issue #58 — a :443 webserver override is a forbidden
    # config shape). 443 was a wrong default that shipped in rendered
    # fleet configs; load() migrates it (see _migrate_stale_http_port).
    http_port: int = 9443
...
class MeshCoreConfig:
    ...
    # Inbound SOURCE-channel allowlist for bridging (ported from MeshForge
    # 8910c156, 2026-09-18). None (default) = every channel EXCEPT MeshCore's
    # Public slot 0. An explicit list is honoured exactly as written, so
    # Public is opt-IN; an explicit EMPTY list means "bridge no channel
    # traffic at all" and is a legitimate DM-only posture, never re-read as
    # "allow all". Overridden per box by MESHANCHOR_MESHCORE_BRIDGE_CHANNELS.
...
class GatewayConfig:
    ...
    def _migrate_stale_http_port(meshtastic_data: dict) -> dict:
        """Migrate the stale http_port=443 default (MeshForge Issue #62
        pattern; parity port of MeshForge `6253d53`).
        ...
        class). 443 was never a valid value for us (MeshForge Issue #58
        treats a :443 webserver override as forbidden), so it is safe to
        treat a saved 443 as the stale default rather than operator
        intent.
        """
