...
class MeshtasticProtobufClient(ProtobufAdminMixin):
    ...
    def _post_toradio(self, to_radio_bytes: bytes) -> bool:
        ...
        # config named — the hop that reached a real radio on 2026-08-09.
        ...
        try:
            ...
            # threshold matches connect_timeout (default 5s)
            with timed_boundary(
    ...
    def _get_fromradio(self) -> Optional[bytes]:
        ...
        try:
            ...
            ctx = self._ssl_ctx if self._config.tls else None
            # threshold matches read_timeout (default 10s) — fromradio polls
            # legitimately wait for queued packets before returning empty.
            with timed_boundary(
