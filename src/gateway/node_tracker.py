...
class UnifiedNodeTracker:
    ...
    def _init_rns_main_thread(self):
        ...
        try:
            ...
            try:
                ...
                # symptom was duplicated `Discovered RNS node ...` log
                # lines and the node's service_type flapping between
                ...
                # is intentionally absent. (Ported from MeshForge.)
                known_aspects = [
    ...
    def _merge_node(self, existing: UnifiedNode, new: UnifiedNode):
        ...
        # (MeshForge 2026-07-21 review C1, ported as the twin).
        # update_pki_status owns the transitions: first-key TOFU, same-key
        # no-op, changed-key CHANGED.
        ...
        try:
            ...
            for node_data in data.get('nodes', []):
                node = UnifiedNode(
                    ...
                    # MeshForge 2026-07-21 review (C2): the loader restored a
                    # handful of fields and silently dropped the rest of
                    # to_dict()'s output — the same writer-with-no-reader
                    # class as service_type, enumerated this time. The
                    # round-trip test now compares the FULL serialized shape
                    # so the next added field cannot regress silently.
                ...
                if NODE_STATE_AVAILABLE and node_data.get('state_machine'):
                    try:
                        ...
                        # Pri-3 (07-23 review, MF twin): is_online was just forced
                        # False ("not heard since restart"), but the machine may
                        # have persisted an ACTIVE state (e.g. ONLINE) that would
                        # make node.state contradict is_online — a not-yet-heard
                        # node reading as live. Reconcile the live claim to
                        # STALE_CACHE (history preserved); first update re-promotes.
                ...
                # modes #4). Found and fixed in MeshForge 2026-07-21, where it
                # made a healthy configured propagation node read as "never
                # heard"; ported here as the identical twin defect.
    ...
    def _save_cache(self):
        ...
        try:
            ...
            try:
                ...
                cache_path = MeshAnchorPaths.rns_nodes_cache_path()
                if cache_path.is_symlink():
                    logger.warning(f"Refusing to write to symlink: {cache_path}")
                else:
                    # atomic_write_text (mkstemp 0600 + fsync + os.replace) is
