...
PORTED from MeshForge ``8910c156`` (2026-09-18), which created the seam this
file is. Before that port MeshAnchor's ``meshcore_handler.py`` sat at 1,499
lines against the 1,500-line MF025 cap with ONE line of headroom, so the
inbound channel guard — the reason any of this happened — could not be added
without a split. MeshForge's MeshCore is disabled on every box, so the seam
was built and proven there first, where a refactor could not break a live
gateway; this box runs MeshCore for real. MF025's hint for this file used to
say "NO upstream seam to port"; that is now false and the hint is updated in
the same commit, because a hint pointing at nothing is the defect it exists
to prevent.

⚠️ On this box the private slot is ``bridge_target_channel`` = 1 and Public is
slot 0, which is why refusing slot 0 inbound makes the rail SYMMETRIC: the
outbound leg has refused slot 0 by design since 2026-05-19 (#35 ead12c7b,
#37 6d9bc395, both of which cured "stacked default-to-zero traps" on the
WRITE path). Inbound was never filtered at all — Public was modeled as an
output hazard, never as an input surface.
