...
def get_identity_path() -> Path:
    """Get path to MeshAnchor gateway identity.

    This is a MeshAnchor-specific file, so uses get_real_user_home()
    (not RNS paths) to store in the real user's config dir.
    """
    return get_real_user_home() / ".config" / "meshanchor" / "gateway_identity"


def create_identities() -> CommandResult:
    """Create RNS and MeshAnchor gateway identities if they don't exist.

    RNS identity: Created by RNS.Identity() in the config directory.
    Gateway identity: MeshAnchor-specific, stored in ~/.config/meshanchor/.
    ...
    # 2. Gateway identity (meshanchor config dir)
...
def get_lxmf_storage_path() -> Path:
    ...
    This is a MeshAnchor-specific directory, so uses get_real_user_home().
    """
    return get_real_user_home() / ".config" / "meshanchor" / "lxmf_storage"
...
def write_config(content: str, backup: bool = True) -> CommandResult:
    ...
    try:
        ...
        if backup and config_path.exists():
            # Second-resolution name: two config writes in the same
            # second used to land on ONE path, and the backup being
            # overwritten was the one holding the true pre-change original.
