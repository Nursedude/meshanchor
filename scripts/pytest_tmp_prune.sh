...
#
# Ported from the MeshForge twin. PREVENTIVE here — read the provenance of each
# claim below before treating any number as MeshAnchor's.
#
# WHY (measured 2026-07-28/29 on the MESHFORGE manager box, NOT on this fleet):
...
# (~79 MB of shmem); dirs accumulated to 37 over two days (3.0 GB) and later to
# 15 dirs / 1.9 GB in a single session, driving user.slice to 82% of its 8 GB
# cap on the box whose reset #8 was a memory-pressure event.
#
...
# suite): /tmp IS a tmpfs, 1.9 GB on a 3.8 GB box — the ceiling is HALF the
# machine's RAM, proportionally worse than the MF manager. ZERO pytest run dirs
# had accumulated there. Insurance against a class observed on the sister
...
# with a false "wrapper_crashed" as the newest line; MF review port 2026-07-28):
...
# diverge on a box with a different system pytest (MF review port 2026-07-28).
# ONE pinned constant, guarded by the drift pin in
# tests/test_pytest_tmp_prune.sh which asks pytest itself: if upstream changes
# LOCK_TIMEOUT the suite fails loudly instead of the pruner silently going
# unsafe again.
