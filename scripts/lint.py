...
- MF024: version SSOT vs pyproject/README badge+heading drift (delegates to scripts/version_consistency_check.py; ported from MeshForge 2026-07-07)
...
class MeshAnchorLinter:
    ...
    def lint_file(self, filepath: str) -> List[LintIssue]:
        ...
        # Check each line. Track the running char offset so the lookahead/
        # lookback rules use THIS line's position, not content.find(line) —
        # which returns the FIRST textual occurrence, silently misclassifying
        # a real violation on a later duplicate line (2026-07-09 frontier
        # review of the gates, ported from MeshForge).
        offset = 0
...
def check_version_consistency(repo_root: str = '.') -> List[LintIssue]:
    """MF024: the version SSOT must agree with pyproject + README badge/heading.
    Prevents version drift (mirrored from MeshForge, the lead repo). Delegates to
    scripts/version_consistency_check.py (byte-identical across the fleet's repos;
    auto-detects the SSOT file)."""
    issues: List[LintIssue] = []
    ...
    if ssot is None:
        return [LintIssue("(version)", 0, Severity.ERROR, "MF024", problems[0])]
    for msg in problems:
...
# ─────────────────────────────────────────────────────────────────────────
# MF025: the file-size ratchet (ported from MeshForge 2026-07-13). CLAUDE.md
# has said "ALWAYS split files exceeding 1,500 lines" since the foundation
# docs — but nothing enforced it. A rule that lives in model memory is a
# house of cards across model handoffs; this compiles it down to the harness
# (model-agnostic-harness principle). Ratchet shape mirrors the MeshForge
# MF018 baseline: known offenders are FROZEN at their 2026-07-13 line counts
# and may only shrink; everything else (and any new file) fails above the
# limit. When an arc splits a baseline file, delete its entry. DO NOT add
# entries to grant new headroom — split the file.
# ─────────────────────────────────────────────────────────────────────────
MF025_LINE_LIMIT = 1_500

# Frozen 2026-07-13. Entries may only shrink or be deleted — NEVER add one to
# grant headroom; split the file. Both original offenders were split under the
# 1,500-line cap 2026-07-14 (prometheus_exporter → metrics_http_handler;
# map_data_collector → _map_collector_config / MapCollectorConfigMixin), so the
...
# MF026: config/state persistence must be ATOMIC. Ported from MeshForge (which
# ported it from the meshing_around_meshforge client repo, 2026-07-16 MED3): a
...
# loses power mid-write — a real event on the fleet's SD-card Pis. Route through
# `utils.paths.atomic_write_text` (temp-in-same-dir + fsync + os.replace).
#
# Two legs: (1) `os.O_TRUNC` BANNED outright. (2) `open(<config path>, "w")` is
# ratcheted against a FROZEN baseline (MF025-style); the hints separate real
# config writes from the repo's benign report/cache/pid/log writes. Baseline only
# SHRINKS; a NEW config write fails. A genuinely-atomic line (writes a temp it
# then os.replace()s) may carry a trailing `# atomic-write-ok` marker.
...
# Frozen 2026-07-16. Entries may only SHRINK (route the write through
# atomic_write_text and drop the entry) — never add one to grant headroom.
MF026_BASELINE = {
...
def main():
    ...
    if args.all:
        ...
        # files someone happened to touch. An instrument reporting "clean"
        # over a scope narrower than its own name is the detector-blind class
        # aimed at our own gate. Ported from MeshForge eed28145, where a raw
        # RNS.Reticulum() (MF019) sat unseen in scripts/ for a full session.
    ...
    # MF024: version SSOT vs pyproject/README drift (repo-level; mirrored from
    # MeshForge). Whole-repo only.
    if not args.staged:
    ...
    # open('w') ratchet). Ported from MeshForge; cheap per-file scan, both modes.
