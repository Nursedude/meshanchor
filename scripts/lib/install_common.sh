...
#
# Ported from MeshForge's scripts/lib/install_common.sh (install-hardening
# parity, 2026-06-23). The `mf_` function prefix is kept verbatim from MeshForge
# so the two libs stay in sync (the prefix is internal collision-avoidance, not
# user-facing — same rationale MeshAnchor kept the MF0xx lint codes).
#
...
# pip existed, swallowed apt/pip output into /dev/null, and printed an
# unconditional `✓` regardless. This library is the shell twin of
# src/utils/pip_install.py — ONE place that:
...
#   scripts/fix_packaging_conflict.sh, scripts/healthcheck.sh

# --------------------------------------------------------------------------
# Transcript — every install run leaves something the operator can paste.
# --------------------------------------------------------------------------
# root  → /var/log/meshanchor/install-<UTC-ts>.log (+ install-latest.log)
# user  → ~/.cache/meshanchor/logs/install-<UTC-ts>.log
...
mf_log_init() {
    ...
    if [[ "${EUID:-$(id -u)}" -eq 0 ]]; then
        dir="/var/log/meshanchor"
    else
        dir="${HOME:-/tmp}/.cache/meshanchor/logs"
    fi
    ...
    if : > "$MF_INSTALL_LOG" 2>/dev/null; then
        ...
        echo "MeshAnchor install transcript: $MF_INSTALL_LOG"
    else
...
# --------------------------------------------------------------------------
# PEP 668 — ONE detector (replaces the divergent copies in install.sh,
...
# output (which would mask pip's exit code). Returns pip's real exit code; the
# CALLER checks it.
# --------------------------------------------------------------------------
mf_pip_install() {
    local py="$1"; shift
...
# optional MESHANCHOR_REF pin (tag/branch/sha). An unresolvable pin HARD-fails
# (returns nonzero): a pinned install silently landing on main is worse than
# stopping. Echoes the resolved HEAD for provenance. A pull failure on an
# existing tree is loud but non-fatal — we record the SHA we kept rather than
# running pip against a half-updated tree under the illusion of success.
# --------------------------------------------------------------------------
mf_git_sync() {
    local url="$1" dir="$2" ref="${MESHANCHOR_REF:-}"
    if [[ -d "$dir/.git" ]]; then
    ...
    if [[ -n "$ref" ]]; then
        ...
        git -C "$dir" checkout -q "$ref" \
            || { echo "ERR: MESHANCHOR_REF '$ref' could not be resolved" >&2; return 1; }
    fi
    local sha
