...
#   */30 * * * * /path/job.sh >/dev/null 2>&1; /opt/meshanchor/scripts/cron_verdict.sh job $?
#
...
# this one in place:
#   */30 * * * * /path/job.sh >$HOME/.local/state/meshforge/cron_out/job.out 2>&1; /opt/meshanchor/scripts/cron_verdict.sh job $?
#
...
# daily cron's single verdict scrolled out ~23.5h after its run and the
# stale-cron detector read healthy crons as "silent: never" — the
# reader/writer retention drift found 2026-07-09 (ported from MeshForge; the
# retention floor must exceed the slowest wired cadence x the detector's
# cadence multiplier). Bound: MAX_LINES + (#names x KEEP_PER_NAME).
...
#
# Path is DELIBERATELY identical to MeshForge's, not renamed to meshanchor: a
# box runs one verdict log ($HOME/cron_verdicts.log, shared by both apps) and so
# must have one capture dir. Two dirs would mean a cron wired by one repo's
# wirer and read by the other repo's verdict script never find each other —
# honest_failure_modes #5 across the twin boundary.
