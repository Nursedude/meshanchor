...
# reliability matrix.
#
# For each fleet box in $HOME/.config/meshanchor/fleet_hosts (or the
# fallback paths fleet_sync.sh uses), ssh in and pull the last
# $LOOKBACK_HOURS of journalctl --user-unit=meshanchor-tracer output
...
#   FLEET_HOSTS=~/.config/meshanchor/fleet_hosts bash scripts/lab_traffic_rollup.sh
#
# Exit codes:
...
local_result=$(journalctl --user-unit=meshanchor-tracer \
...
while IFS= read -r raw; do
    ...
    result=$(ssh -o BatchMode=yes -o ConnectTimeout=8 "$host" \
        "journalctl --user-unit=meshanchor-tracer --since '${LOOKBACK_HOURS} hours ago' \
...
if [[ "$rows" -eq 0 ]]; then
    ...
    echo "       Is meshanchor-tracer.timer enabled on a tester box?" >&2
    exit 3
fi
