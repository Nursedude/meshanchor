...
Ported from MeshForge (the lead repo for the role/preset provisioner) and adapted
to MeshAnchor's paths/topology. MeshForge's fleet_roles.yaml marks the
`meshanchor-noc` role `provisioned_by: meshanchor` — MeshAnchor owns the
convergence of its OWN units; this is that owner.

Reads the declarative role catalog (`docs/fleet_roles.yaml`) and the box's role
(`~/.config/meshanchor/deployment.json` → `role`), then brings local systemd unit
...
operation (no raw systemctl here).
  ...
  v1 scope:
    ...
    - cross-cutting permission foundation: converged via utils.fleet_foundation
    - external roles (provisioned_by:*) and singletons: reported, not enforced
  (The MeshForge map config_delta assertions are NOT ported — they are coupled to
   MeshForge's map internals; MeshAnchor's map convergence is a later slice.)
...
# via $MESHANCHOR_SSH (no key/host hardcoded here). The operator's ssh
...
# per box — Issue #69). MeshAnchor's rnsd-rival policy depends on which boxes
# run an MA-owned rnsd vs join a shared instance — left EMPTY until the Phase-B
# role topology pins it (the masking block below only fires for an rnsd-enabled
# role, so an empty set is a safe no-op default; masking is additive).
