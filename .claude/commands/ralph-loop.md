...
2. **Execute**
   - Do the next step
   - Follow the auto-loaded MeshAnchor rules (CLAUDE.md + `.claude/rules/security.md`)
   - Walk the honest-failure-modes checklist over every error path you write
     (degraded state must never read as a valid value — MeshForge Issue #80 lesson)
...
- [ ] Pushed: `git push origin main` (then pull meshanchor-server + any other clones)
