# When Your Mesh Gets Busy: How MeshAnchor Fails Over Before You Lose Packets
...
We've been building MeshAnchor as an open-source NOC that bridges Meshtastic and Reticulum mesh networks. When Nursedude described this problem from his deployments, the fix couldn't be "tell people to transmit less." The fix had to be automatic and invisible to the operator.

## Two Radios, One State Machine

The solution: run two meshtasticd instances on separate radios (ports 4403 and 4404) and let MeshAnchor manage which one is actively transmitting.
...
## Why This Matters Beyond MeshAnchor
...
The Meshtastic firmware team may eventually address the 25% throttle differently. Firmware changes take time and affect every device. MeshAnchor's approach works today, with current firmware, using hardware operators already own. You just need a second radio and a config change.

The failover system ships with MeshAnchor v0.5.4-beta. It's off by default — enable it through the TUI or by setting `failover.enabled: true` in your gateway config. The test suite covers every state transition, rate limit, and edge case. What it needs now is field time with real RF on real channels.
