# MeshAnchor Session Notes - MOC Broker Configuration

**Date**: 2026-02-09
**Branch**: `claude/configure-meshanchor-broker-h1VTf`
**Version**: v0.5.2-beta
**Tests**: 3926 passed, 1 failed (pre-existing rnsd sandbox), 19 skipped

## Session Focus: MOC1 Broker Configuration

Configured MeshAnchor broker templates for a two-node MQTT-bridged deployment:
...
MOC1 (Pi5, Meshtoad, LongFast)          MOC2 (Pi HAT, ShortTurbo)
  ...
  ├── MeshAnchor NOC + MQTT subscriber      ├── MeshAnchor Gateway
  └── Web Client (:9443)                   └── meshanchor ch ↔ RNS bridge
           │                                       │
           └────── MQTT over LAN ──────────────────┘
```
...
| `templates/meshanchor-presets/fleet-host-1-broker.yaml` | **NEW** | Full MOC1 preset: Meshtoad + LongFast + MQTT uplink + mosquitto broker + deployment checklist |
...
4. **MOC2 stays RNS/NomadNet** with MeshtasticInterface on ShortTurbo, meshanchor channel bridged to RNS
...
5. Run MeshAnchor TUI → MQTT Broker Manager → Setup Private Broker (LongFast, US)
...
5. Set up MeshAnchor gateway bridge (message_bridge mode)
...
- [ ] Verify MQTT flow: `mosquitto_sub -h localhost -u meshanchor -P <pw> -t "msh/#" -v`
- [ ] Configure MOC2 to point at MOC1 broker
- [ ] Test cross-mesh message: LongFast → MQTT → ShortTurbo
- [ ] Test RNS bridge: ShortTurbo → MeshAnchor Gateway → RNS/NomadNet
