...
**2026-05-04 (Phase 5.5 MERGED + map_data_collector split — PR pending)**:
- PR #25 merged into main as merge commit `737a1cc1`. Phase 5.5 shipped the profile-aware health helper and wired it through health_score / active_health_probe / service_menu. CI canary on first push: existing `tests/test_active_health_probe.py::TestCreateGatewayHealthProbe` didn't patch `_active_profile`, fixed in commit `36df3907` by pinning a FULL profile inside both legacy tests. Memory feedback updated: environment-dependent default behaviour is the new failure-mode pattern to watch for.
- **Branch**: `claude/mc-collector-split` off main. Pure refactor — no behaviour change, no new tests.
- **Files changed** (2):
  - `src/utils/_map_collector_meshtastic.py` (NEW, 515 lines): `MeshtasticDataCollectorMixin` containing 8 methods extracted from `MapDataCollector` — `_collect_meshtasticd`, `_collect_via_http`, `_collect_via_tcp_interface`, `_collect_direct_radio`, `_parse_tcp_node`, `_extract_node_info_without_position`, `_collect_via_cli`, `_parse_meshtastic_info`. Mirrors the existing `_map_collector_rns.py` mixin pattern. Module-level `try/except ImportError` for the meshtastic_http / meshtastic_connection imports (preserves `_HAS_MESHTASTIC_CONN` flag semantics from the host file).
  - `src/utils/map_data_collector.py` (1529 → 1054, -475): removed the 8 methods; class definition flips to `class MapDataCollector(MeshtasticDataCollectorMixin, RNSDataCollectorMixin)`. The Meshtastic try/except import block is gone from this file too — it lives in the mixin module now.
- **Why split just Meshtastic, not all five sources**: the file's been over the 1500 cap since well before Phase 6 — the goal is to get under the cap with the minimum-risk change, not refactor for refactor's sake. Meshtastic is by far the largest per-source block (475 lines vs 84 for node_tracker, 78 for MeshCore, 33 for MQTT, 146 for AREDN). Extracting it alone takes the file from 1529 → 1054, comfortably under cap. Other source extractions are independently mergeable as future cleanup if/when they're justified — but a 1054-line file isn't crying out for further surgery today.
- **Gates green**: `python3 scripts/lint.py --all` exit 0; canonical `tests/test_map_data_collector.py` = 8 passed (all 4 test classes still functional through the mixin); full suite `pytest tests/ --ignore=tests/test_bridge_integration.py` = **3107 passed** (zero delta — pure refactor adds no tests, removes no behaviour). Same two pre-existing dev-box flakes — not split regressions.
