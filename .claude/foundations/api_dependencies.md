...
**Breaking Change Impact**: 3 files
...
| File | Usage | Notes |
|------|-------|-------|
| `gtk_ui/panels/*.py` | Service status display | Multiple panels |
| `tui/panes/dashboard.py` | TUI dashboard | Shows status icons |
| `commands/rns.py` | RNS commands | Pre-flight checks |
...
| File | Usage | Notes |
|------|-------|-------|
| `utils/claude_assistant.py` | Log analysis | Iterates suggestions |
| `gateway/rns_bridge.py` | Error handling | Reports symptoms |
| `gtk_ui/panels/diagnostics.py` | UI display | Shows suggestions[0] with guard |
| `tests/test_diagnostics.py` | Multiple tests | Tests return types |
